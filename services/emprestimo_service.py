...
class EmprestimoService:
    ...
    def inserir_emprestimo(self, emprestimo_ui):
        if emprestimo_ui.cd_tipo_uniforme.currentText() != 'Selecione um item' \
                and emprestimo_ui.selected_funcionario is not None:
            unforme = self.uniforme_repository.select_uniforme_by_name(emprestimo_ui.cd_tipo_uniforme.currentText())
            try:
                self.emprestimo_repository.insert_emprestimo(emprestimo_ui.selected_funcionario, unforme)
                QMessageBox.information(emprestimo_ui, 'Empréstimos', 'Empréstimo cadastrado com sucesso')
            except Exception as e:
                QMessageBox.warning(emprestimo_ui, 'Empréstimos', f'Erro ao cadastrar empréstimo.\nErro : {e}')
        elif emprestimo_ui.cd_tipo_uniforme.currentText() == 'Selecione um item' \
                and emprestimo_ui.selected_funcionario is not None:
            QMessageBox.warning(emprestimo_ui, 'Empréstimos', f'Selecione um uniforme')
        elif emprestimo_ui.cd_tipo_uniforme.currentText() != 'Selecione um item' \
                and emprestimo_ui.cd_tipo_uniforme.selected_funcionario is None:
            QMessageBox.warning(emprestimo_ui, 'Empréstimos', f'Selecione um funcionário')
        else:
            QMessageBox.warning(emprestimo_ui, 'Empéstimos', f'Selecione um funcionário e um uniforme')
    ...
    def finalize_emprestimo(self, main_window):
        ...
        msg_box.setText('Tem certeza que deseja finalizar este emmpréstimo?')
